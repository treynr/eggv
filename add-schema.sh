...
## file: add-schema.sh
## desc: Helper script to load the variant schema and some necessary data 
##       into the GeneWeaver database.
## vers: 0.1.12
## auth: TR
#
...
psql "$connect" -f "$sdir/variant-schema.sql"
psql "$connect" -f "$sdir/insert-variant-types.sql"
psql "$connect" -f "$sdir/insert-genome-builds.sql"
psql "$connect" -f "$sdir/insert-variant-genedb-entry.sql"
