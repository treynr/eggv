...
def format_neo4j_genes(input: str, output: str, build: str) -> None:
    ...
    df = df.rename(columns={'gene_id': 'gene_id:ID(Gene)'})
...
def run_hg38_variant_formatting(
    ...
    outdir: str = globe._dir_hg38_variant_neo,
    **kwargs
) -> Future:
    """
    28 min.
    ...
    for fl in Path(indir).iterdir():
        ...
        future = client.submit(format_neo4j_variants, fl, output, 'hg38')
...
def run_mm10_variant_formatting(
    ...
    output: str = globe._fp_mm10_variant_neo,
    **kwargs
) -> Future:
    """
    28 min.
    ...
    return client.submit(format_neo4j_variants, input, output, 'mm10')


def run_hg38_gene_formatting(
    client: Client,
    input: str = globe._fp_hg38_gene_processed,
    output: str = globe._fp_hg38_gene_neo,
    **kwargs
) -> Future:
    """
    28 min.
    ...
    return client.submit(format_neo4j_genes, input, output, 'hg38')


def run_mm10_gene_formatting(
    client: Client,
    input: str = globe._fp_mm10_gene_processed,
    output: str = globe._fp_mm10_gene_neo,
    **kwargs
) -> Future:
    """
    28 min.
    ...
    return client.submit(format_neo4j_genes, input, output, 'mm10')
